...
/// HTML-escape text destined for element content. Same std-only helpers the
/// server's markdown.rs exports; kept local so this crate builds the page
/// with no dependency on the server's Markdown engine. Keep the copies in
/// lockstep so /news renders identical bytes from either repo.
