...
// store.rs, feeds.rs, and fetch.rs live in this crate's library, where the
// zero-trust server can consume the same code for the /news read path. This
// binary only touches the write half: init_db, insert_item, cleanup, the
// source catalog, and the fetch walk. Library items are public, so nothing
// here needs the dead-code allowances the monolith's shared-file includes did.
use news_aggregator::{feeds, fetch, store};
